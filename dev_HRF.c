...
void HRF_send_OOK_msg(uint8_t relayState) {
	...
	HRF_reg_Wn(buf + 4, 0, 12);						// Send message (without preamble)

	for (i = 0; i < 8; ++i) {						// Send few more same messages
...
uint8_t* HRF_make_FSK_msg(uint8_t manufacturerId, uint8_t encryptionId, uint8_t productId, uint32_t sensorId, uint8_t paramNum, ...) {
...
void msgNextState(uint8_t encryptionId, uint8_t productId, uint8_t manufacturerId, uint32_t sensorId, msg_t *msgPtr) {	// Switch and initialize next state
	...
	/******************* finish reading RECORDS  ********************/
		...
		case S_CRC:							// Check CRC
			msgPtr->state = S_FINISH;
			if ( (int16_t)msgPtr->value == crc(msgPtr->buf + MSG_ENCR_START, msgPtr->bufCnt - (MSG_ENCR_START+2)) ) {
				printf("OK\n");
				if (msgPtr->gotJoin) {
